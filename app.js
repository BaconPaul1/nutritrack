...
window.addEventListener('DOMContentLoaded', () => {
  console.log("NutriTrack v2.10 Loaded (Multi-Photo Support)");
  load();
  if (State.profile) {
    showApp();
  } else {
...
function buildExerciseHistory() {
  const allLogs = [];
  Object.keys(State.exercise).forEach(date => {
    State.exercise[date].forEach((log, index) => {
      allLogs.push({ ...log, date, originalIndex: index });
    });
  });
  ...
  return `<div class="card" style="margin-top:16px; padding:0; overflow:hidden;">
    <div style="padding:14px 18px; border-bottom:1px solid var(--border); background:rgba(255,255,255,0.02)">
      <span class="card-title" style="margin:0">運動記錄歷史</span>
    </div>
    <div style="max-height: 400px; overflow-y: auto;">
      ${sorted.map((l) => `
        <div class="exercise-log-entry" style="padding:12px 18px; border-bottom:1px solid rgba(255,255,255,0.05)">
          <div style="display:flex; flex-direction:column; gap:2px;">
            <div style="font-size:11px; color:var(--text-muted); font-weight:600">${l.date}</div>
            <div style="font-size:14px; font-weight:600">${l.icon || '🏃'} ${l.name}</div>
            <div style="font-size:12px; color:var(--text-secondary)">
              ${l.type === 'cardio' ? `${l.duration} 分鐘` : `${l.sets}組 × ${l.reps}次 × ${l.weight}kg`}
            </div>
          </div>
          <div style="display:flex; align-items:center; gap:12px">
            ${l.burned ? `<span style="font-size:13px; font-weight:700; color:var(--accent-green)">-${l.burned} kcal</span>` : `<span class="badge badge-teal" style="font-size:10px; padding:2px 6px">肌力</span>`}
            <button class="btn-icon" onclick="removeExercise(${l.originalIndex}, '${l.date}')" style="width:28px; height:28px; font-size:12px">✕</button>
          </div>
        </div>
      `).join('')}
    </div>
...
function removeExercise(idx, date) {
  const targetDate = date || State.currentDate;
  if (!State.exercise[targetDate]) return;
  State.exercise[targetDate].splice(idx, 1);
  save();
  navigate('exercise');
}
...
async function handleAIImage(event) {
  ...
  try {
    if (files.length === 1) {
      _aiPendingBase64 = await resizeAndCompressImage(files[0]);
    } else {
      // Merge multiple images
      _aiPendingBase64 = await mergeImagesIntoOne(files);
    }
...
/**
 * Merges multiple files into a single collage (max 4 images)
 */
async function mergeImagesIntoOne(files) {
  ...
  images.forEach((img, i) => {
    const x = (i % 2) * singleW;
    const y = Math.floor(i / 2) * singleH;
    
    // Draw with "cover" logic manually
    const ratio = Math.max(singleW / img.width, singleH / img.height);
    const nw = img.width * ratio;
    const nh = img.height * ratio;
    const offX = (singleW - nw) / 2;
    const offY = (singleH - nh) / 2;
    
    ctx.drawImage(img, x + offX, y + offY, nw, nh);
    
    // Optional: Draw a thin white border between images
    ctx.strokeStyle = '#fff';
    ctx.lineWidth = 2;
